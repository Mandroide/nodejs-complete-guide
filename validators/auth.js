...
// TODO 2024-12-30 Replace model with Seller
const Seller = require("../models/Order");

exports.login = [
    // body('asWhat')
    //     .exists({checkFalsy: true}).withMessage('You must choose as what you would be signed in')
    //     .isIn(['asUser', 'asSeller']).withMessage('You can only sign in as a User or as a Seller'),
    body('email')
        .exists({checkFalsy: true}).withMessage('You must type an email')
        .isEmail().withMessage('You must type a proper email'),
    body('password')
        .notEmpty({ignore_whitespace: false}).withMessage('You must type a password'),
]
